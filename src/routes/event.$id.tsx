...
type EventFunctionRow = Database["public"]["Tables"]["event_functions"]["Row"];
...
type EventInvitationRow = Database["public"]["Tables"]["event_invitations"]["Row"];
type ProfileRow = Database["public"]["Tables"]["profiles"]["Row"];

type MemberWithProfile = EventMemberRow & { profile?: ProfileRow | null };
      ...
      <div className="container mx-auto px-4 py-8">
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-3">
                  ...
                  <span className="rounded-full bg-background/15 px-3 py-1 text-xs uppercase tracking-wide backdrop-blur-sm">
                    {categoryLabel(event.category)}
                  </span>
                  <span className="rounded-full gradient-gold px-3 py-1 text-xs font-semibold text-foreground">
                    {role}
                  </span>
                </div>
                ...
                {event.description && (
                  <p className="mt-2 max-w-2xl text-primary-foreground/85">{event.description}</p>
                )}
              ...
              <div className="rounded-xl bg-background/15 p-4 backdrop-blur-sm">
                ...
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(event.share_code);
                    toast.success("Copied!");
                  }}
                  className="mt-1 flex items-center gap-2 font-mono text-2xl font-bold tracking-widest text-gold"
                >
                  {event.share_code} <Copy className="h-4 w-4" />
                </button>
                <p className="mt-1 text-xs text-primary-foreground/70">
                  Share with family & photographer
                </p>
              </div>
            </div>
          </div>
        ...
        <Tabs value={tab} onValueChange={setTab} className="mt-8">
          ...
          <TabsContent value="timeline">
            <TimelineTab eventId={id} canEdit={isHost} />
          </TabsContent>
          <TabsContent value="guests">
            <GuestsTab eventId={id} canEdit={canManageGuests} canDelete={isHost} />
          </TabsContent>
          <TabsContent value="photos">
            <PhotosTab eventId={id} userId={user.id} canUpload={canUpload} isHost={isHost} />
          </TabsContent>
          <TabsContent value="members">
            <MembersTab eventId={id} hostId={event.host_id} isHost={isHost} />
          </TabsContent>
        </Tabs>
      </div>
    </div>
...
function TimelineTab({ eventId, canEdit }: { eventId: string; canEdit: boolean }) {
  const [items, setItems] = useState<EventFunctionRow[]>([]);
    ...
    <div className="mt-6">
      {canEdit && (
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle className="font-serif text-2xl">Add a function</DialogTitle>
            </DialogHeader>
      ...
      {items.length === 0 ? (
        <Card className="p-12 text-center text-muted-foreground">
          ...
          <p className="mt-3">No functions yet.</p>
        </Card>
      ) : (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    <div className="text-xs font-semibold uppercase tracking-wide text-primary">
                      {new Date(it.starts_at).toLocaleString("en-IN", {
                        dateStyle: "medium",
                        timeStyle: "short",
                      })}
                    </div>
...
function GuestsTab({
  ...
  const toggleArrived = async (g: GuestRow) => {
    const arrived_at = g.arrived_at ? null : new Date().toISOString();
    const { error } = await supabase
      .from("guests")
      .update({ arrived_at, rsvp_status: arrived_at ? "arrived" : g.rsvp_status })
      .eq("id", g.id);
    if (error) return toast.error(error.message);
    load();
  };
    ...
    <div className="mt-6">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div className="flex gap-3">
          <Card className="px-4 py-2">
            ...
            <div className="font-serif text-2xl font-semibold">{guests.length}</div>
          </Card>
          <Card className="px-4 py-2">
            <div className="text-xs text-muted-foreground">Arrived</div>
            <div className="font-serif text-2xl font-semibold text-primary">{arrivedCount}</div>
          </Card>
        </div>
        {canEdit && (
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle className="font-serif text-2xl">Add guest</DialogTitle>
              </DialogHeader>
              <form onSubmit={add} className="space-y-3">
                <div>
                  <Label>Name</Label>
      ...
      {guests.length === 0 ? (
        <Card className="p-12 text-center text-muted-foreground">
          ...
          <p className="mt-3">No guests added yet.</p>
        </Card>
      ) : (
        <div className="grid gap-3 md:grid-cols-2">
          {guests.map((g) => (
            <Card
              key={g.id}
              className={`p-4 transition-smooth ${g.arrived_at ? "border-primary/40 bg-primary/5" : ""}`}
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  ...
                  {g.arrived_at && (
                    <div className="mt-1 text-xs font-medium text-primary">
                      Arrived{" "}
                      {new Date(g.arrived_at).toLocaleTimeString([], {
                        hour: "2-digit",
                        minute: "2-digit",
                      })}
                    </div>
                ...
                <div className="flex flex-col gap-2">
                  {canEdit && (
                    <Button
                      size="sm"
                      variant={g.arrived_at ? "default" : "outline"}
                      onClick={() => toggleArrived(g)}
                    >
                      <Check className="h-3.5 w-3.5" />
                      {g.arrived_at ? "Here" : "Mark"}
                    </Button>
                  )}
                  {canDelete && (
...
function PhotosTab({
  ...
  const load = async () => {
    ...
    if (data && data.length) {
      ...
      await Promise.all(
        data.map(async (p: EventPhotoRow) => {
  ...
  const handleFiles = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of Array.from(files)) {
      ...
      const { error: insErr } = await supabase
        .from("event_photos")
        .insert({ event_id: eventId, uploader_id: userId, storage_path: path });
  ...
  const remove = async (p: EventPhotoRow) => {
          ...
          {photos.map((p) => (
            ...
            >
              ...
              {(isHost || p.uploader_id === userId) && (
                <button
                  onClick={() => remove(p)}
                  className="absolute right-2 top-2 rounded-full bg-destructive/80 p-1.5 text-destructive-foreground opacity-0 backdrop-blur-sm transition group-hover:opacity-100"
                >
...
function MembersTab({
  ...
  const [invites, setInvites] = useState<EventInvitationRow[]>([]);
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({
    email: "",
    role: "photographer" as "photographer" | "family",
  });
  const [busy, setBusy] = useState(false);

  const load = async () => {
    const { data: m } = await supabase
      .from<EventMemberRow>("event_members")
      .select("*")
      .eq("event_id", eventId);
    if (m) {
  ...
  const sendInvite = async (e: React.FormEvent) => {
    e.preventDefault();
    const parsed = z
      .object({ email: z.string().trim().email(), role: z.enum(["photographer", "family"]) })
      .safeParse(form);
        ...
        <div>
          <div className="mb-4 flex items-center justify-between">
            ...
            <Dialog open={open} onOpenChange={setOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle className="font-serif text-2xl">Invite someone</DialogTitle>
                </DialogHeader>
                  ...
                  <div>
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-2">
                      <button
                        ...
                        className={`rounded-lg border-2 p-3 text-left transition ${form.role === "photographer" ? "border-primary bg-primary/5" : "border-border"}`}
                      >
                        <Camera className="h-4 w-4" />
                        <div className="mt-1 font-medium">Photographer</div>
                        <div className="text-xs text-muted-foreground">Can upload photos</div>
                      </button>
                      <button
                        type="button"
                        onClick={() => setForm({ ...form, role: "family" })}
                        className={`rounded-lg border-2 p-3 text-left transition ${form.role === "family" ? "border-primary bg-primary/5" : "border-border"}`}
                      >
                        <Users className="h-4 w-4" />
                        <div className="mt-1 font-medium">Family</div>
                        <div className="text-xs text-muted-foreground">Can manage guests</div>
                      </button>
                    </div>
                  </div>
                  <p className="rounded-md bg-secondary/60 p-3 text-xs text-muted-foreground">
                    After creating, an invite link is copied to your clipboard. Share it with them —
                    they'll sign in with the same email to accept.
                  </p>
              ...
              {invites.map((inv) => (
                <Card key={inv.id} className="flex items-center justify-between p-3">
                  ...
                  <div className="flex gap-1">
                    {!inv.used_at && (
                      <Button size="icon" variant="ghost" onClick={() => copyInvite(inv.token)}>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {members.map((m) => (
            <Card key={m.id} className="flex items-center justify-between p-4">
              ...
              <span
                className={`rounded-full px-3 py-1 text-xs font-semibold capitalize ${m.user_id === hostId ? "gradient-gold text-foreground" : "bg-secondary text-secondary-foreground"}`}
              >
                {m.role}
              </span>
            </Card>
