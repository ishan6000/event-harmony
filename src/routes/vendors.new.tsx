...
const schema = z.object({
  ...
  category: z.enum(["photography", "catering", "decor", "music", "makeup", "venue", "planner", "other"]),
...
function NewVendor() {
  ...
  const [form, setForm] = useState({ business_name: "", category: "photography" as const, description: "", city: "", phone: "", email: "", website: "" });
  ...
  return (
    <div className="min-h-screen bg-secondary/20">
      ...
      <div className="container mx-auto max-w-2xl px-4 py-10">
        <Button variant="ghost" size="sm" asChild className="mb-4"><Link to="/vendors"><ArrowLeft className="h-4 w-4" /> Back to vendors</Link></Button>
        <div className="rounded-2xl border border-border bg-card p-6 shadow-elegant md:p-8">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Reach hosts planning their next event.</p>
          <form onSubmit={submit} className="mt-6 space-y-4">
            ...
            <div><Label>About</Label><Textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} maxLength={500} placeholder="Tell hosts what makes you special…" rows={3} /></div>
            <div className="grid gap-4 sm:grid-cols-2">
              <div><Label>City</Label><Input value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} placeholder="Mumbai" /></div>
            ...
            <Button type="submit" variant="hero" className="w-full" disabled={busy}>{busy ? "Creating…" : "Publish listing"}</Button>
          </form>
        </div>
      </div>
    </div>
